...
const FINAL_TWIST_COMPONENTS = {
  PREFIX: [
    'WITH BURST POWER ONLY', 'WHILE COMMS LAG 90 SECONDS', 'UNDER LIVE GLOBAL BROADCAST',
    'WHILE GEAR RECONFIGURES RANDOMLY', 'WITH THE TEAM SPLIT ACROSS ZONES', 'UNDER CONSTANT AFTERSHOCKS',
    'WHILE ACTION WINDOWS LAST 2 MINUTES', 'WITH RULES THAT CHANGE EACH HOUR',
    'WHILE DIGITAL SYSTEMS ARE PARTLY CORRUPTED', 'WITH HOSTILE WEATHER TRACKING YOU',
    'WHILE TRANSLATIONS DROP MEANING', 'UNDER CIVILIAN OVERSIGHT',
    'WITH SUPPLIES THAT DEGRADE EACH PHASE', 'WHILE KEY LOCATIONS SHIFT',
    'UNDER A NON-LETHAL PROTOCOL', 'WHILE MAP UPDATES ARE 30 MINUTES OLD',
    'WITH ONE SHARED POWER SOURCE', 'WHILE RUMORS CREATE FALSE TARGETS',
    'WITH YOUR TOP EXPERT OFFLINE', 'UNDER INTERMITTENT ZERO VISIBILITY'
  ],
  MIDDLE: [
    'AND EVERY PLAN NEEDS AN EVAC ROUTE', 'AND YOU CANNOT USE DIRECT FORCE',
    'AND YOU MUST HOLD A FRAGILE ALLIANCE', 'AND EACH MOVE MUST BE AUDITABLE',
    'AND YOU ACT ONLY ON MATCHING SIGNALS', 'AND EACH FAILURE ADDS A SUB-CRISIS',
    'AND CONTROL PASSES EVERY 10 MINUTES', 'AND HALF YOUR SENSOR FEED IS FAKE',
    'AND YOU CANNOT REUSE A STRATEGY', 'AND CRITICAL TARGETS KEEP MOVING',
    'AND EACH SUCCESS REVEALS A HARDER STEP', 'AND YOU MUST PROTECT HOSTILE WITNESSES',
    'AND INSTRUCTIONS ARE LIMITED TO 5 WORDS', 'AND YOUR BEST TOOL HAS A LONG COOLDOWN',
    'AND TEAM TIMELINES ARE DESYNCED', 'AND EACH DECISION SHIFTS THE MARKET',
    'AND YOU MUST PREVENT ALLY PANIC', 'AND OPPONENTS PREDICT YOUR LAST MOVE'
  ],
  SUFFIX: [
    'WHILE MICRO-RIFTS OPEN', 'BEFORE THE NEXT SOLAR STORM', 'WHILE THE CROWD VOTES ON PRIORITIES',
    'AS FUEL DROPS BELOW 10%', 'WHILE A FALSE-FLAG OP FRAMES YOU',
    'WITH ANALOG BACKUPS ONLY', 'AS GRAVITY FLUCTUATES',
    'WHILE HISTORY STARTS UNDOING ITSELF', 'AS LANGUAGE DRIFTS BY REGION',
    'WITH EVIDENCE TRAILS SELF-DELETING', 'AS MORAL TRADEOFFS GET WORSE',
    'WITH EACH HOUR CUTTING YOUR RANGE', 'AS ALLIES SPLIT INTO RIVALS',
    'WHILE A MYTHIC ANOMALY KILLS MODERN TECH', 'AS TIME DESYNC CREATES DUPLICATE THREATS',
    'WITH ONE EXTRACTION WINDOW LEFT', 'AS A SECOND CRISIS IGNITES',
